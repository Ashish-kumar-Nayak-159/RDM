...
export class RdmGuestLoginComponent implements OnInit {
  ...
  onSignIn() {
    if (this.otpForm.email_otp) {
      ...
      this.subscriptions.push(this.commonService.userSignIn(this.otpForm).subscribe(
        async (response: any) => {
  ...
  onSignUp() {
    if (this.loginForm.email && this.loginForm.name && this.loginForm.org) {
      ...
      // }
      this.subscriptions.push(this.commonService.userSignUp(this.loginForm).subscribe(
        async (response: any) => {
