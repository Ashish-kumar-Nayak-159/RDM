/*!
 * Copyright 2013-2020 Start Bootstrap
 * Licensed under MIT (https://github.com/StartBootstrap/startbootstrap-sb-admin-2/blob/master/LICENSE)
 */

(function ($) {
  "use strict"; // Start of use strict
  // Toggle the side navigation
  // $("#sidebarToggle").on('click', function(e) {
  //   $("body").toggleClass("sidebar-toggled");
  //   $(".sidebar").toggleClass("toggled");
  //   if ($(".sidebar").hasClass("toggled")) {
	//   $('.container-fluid').removeClass( "sb-notoggle" );
	//   $('.container-fluid').addClass( "sb-toggle" );
  //   }
	// if (!$(".sidebar").hasClass("toggled")) {
	//   $('.container-fluid').addClass( "sb-notoggle" );
	//   $('.container-fluid').removeClass( "sb-toggle" );
  //   }
  // });
  // Toggle the side navigation
  // $("#sidebarToggleTop").on('click', function(e) {
  //   $("body").toggleClass("sidebar-toggled");
  //   $(".sidebar").toggleClass("toggled");
  //   if ($('.sidebar').hasClass('toggled')) {
  //     // $(".sidebar .collapse").collapse("hide");
  //     $('.container-fluid').addClass('sb-toggle');
  //     $('.container-fluid').removeClass('sb-notoggle');
  //   }
  //   if (!$('.sidebar').hasClass('toggled')) {
  //     // $(".sidebar .collapse").collapse("show");
  //     $('.container-fluid').removeClass('sb-toggle');
  //     $('.container-fluid').addClass('sb-notoggle');
  //   }
  // });
   // Toggle the side navigation
  //$("#sidebarToggleTop").on('click', function(e) {
  //  $("body").toggleClass("sidebar-toggled");
  //  $(".sidebar").toggleClass("toggled");
  //  if ($(".sidebar").hasClass("toggled")) {
	//  $('.container-fluid').addClass( "sb-collapse" );
	//  $('.container-fluid').removeClass( "sb-toggle" );
   // }
	//if (!$(".sidebar").hasClass("toggled")) {
	//  $('.container-fluid').removeClass( "sb-collapse" );
	//  $('.container-fluid').addClass( "sb-toggle" );
  //  }
  //});
  // Toggle the side navigation1
  // $("#sidebar1Toggle").on('click', function(e) {
  //   $(".sidebar1").toggleClass("toggled");
  //   if ($(".sidebar1").hasClass("toggled")) {
	//   $('.container1-fluid').removeClass( "sb1-notoggle" );
	//   $('.container1-fluid').addClass( "sb1-toggle" );
  //   }
	// if (!$(".sidebar1").hasClass("toggled")) {
	//   $('.container1-fluid').addClass( "sb1-notoggle" );
	//   $('.container1-fluid').removeClass( "sb1-toggle" );
  //   }
  // });
  // Toggle the side navigation1
  // $("#sidebar1ToggleTop").on('click', function(e) {
  //   $(".sidebar1").toggleClass("toggled");
  //   if ($(".sidebar1").hasClass("toggled")) {
	//   $('.container1-fluid').addClass( "sb1-collapse" );
	//   $('.container1-fluid').removeClass( "sb1-toggle" );
  //   }
	// if (!$(".sidebar1").hasClass("toggled")) {
	//   $('.container1-fluid').removeClass( "sb1-collapse" );
	//   $('.container1-fluid').addClass( "sb1-toggle" );
  //   }
  // });

    if (($(window).width() > 768) && $(".sidebar").hasClass("toggled")) {
      $('.container-fluid').removeClass( "sb-collapse" );
      $('.container-fluid').removeClass( "sb-notoggle" );
      $('.container-fluid').addClass( "sb-toggle" );
    }
    if (($(window).width() > 768) && !$(".sidebar").hasClass("toggled")) {
      $('.container-fluid').removeClass( "sb-collapse" );
      $('.container-fluid').removeClass( "sb-toggle" );
      $('.container-fluid').addClass( "sb-notoggle" );
    }
  	if (($(window).width() < 768) && $(".sidebar").hasClass("toggled")) {
      $('.container-fluid').removeClass( "sb-collapse" );
      $('.container-fluid').removeClass( "sb-notoggle" );
      $('.container-fluid').removeClass( "sb-toggle" );
    }
    if (($(window).width() < 768) && !$(".sidebar").hasClass("toggled")) {
      $('.container-fluid').removeClass( "sb-collapse" );
      $('.container-fluid').addClass( "sb-toggle" );
      $('.container-fluid').removeClass( "sb-notoggle" );
    }
	  if (($(window).width() > 768) && $(".sidebar1").hasClass("toggled")) {
      $('.container1-fluid').removeClass( "sb1-notoggle" );
      $('.container1-fluid').addClass( "sb1-toggle" );
    }
    if (($(window).width() > 768) && !$(".sidebar1").hasClass("toggled")) {
      $('.container1-fluid').addClass( "sb1-notoggle" );
      $('.container1-fluid').removeClass( "sb1-toggle" );
    }
	  if (($(window).width() < 768) && $(".sidebar1").hasClass("toggled")) {
      $('.container1-fluid').removeClass( "sb1-notoggle" );
      $('.container1-fluid').removeClass( "sb1-toggle" );
    }
    if (($(window).width() < 768) && !$(".sidebar1").hasClass("toggled")) {
      $('.container1-fluid').addClass( "sb1-toggle" );
      $('.container1-fluid').removeClass( "sb1-notoggle" );
    }

  // Close any open menu accordions when window is resized below 768px
  $(window).resize(function() {
      if (($(window).width() > 768) && $(".sidebar").hasClass("toggled")) {
        $('.container-fluid').removeClass( "sb-collapse" );
        $('.container-fluid').removeClass( "sb-notoggle" );
        $('.container-fluid').addClass( "sb-toggle" );
      }
      if (($(window).width() > 768) && !$(".sidebar").hasClass("toggled")) {
        $('.container-fluid').removeClass( "sb-collapse" );
        $('.container-fluid').removeClass( "sb-toggle" );
        $('.container-fluid').addClass( "sb-notoggle" );
      }
      if (($(window).width() < 768) && $(".sidebar").hasClass("toggled")) {
        $('.sidebar .collapse').collapse('hide');
        $('.container-fluid').removeClass( "sb-collapse" );
        $('.container-fluid').removeClass( "sb-notoggle" );
        $('.container-fluid').removeClass( "sb-toggle" );
      }
      if (($(window).width() < 768) && !$(".sidebar").hasClass("toggled")) {
        $('.sidebar .collapse').collapse('hide');
        $('.container-fluid').removeClass( "sb-collapse" );
        $('.container-fluid').addClass( "sb-toggle" );
        $('.container-fluid').removeClass( "sb-notoggle" );
      }
      if (($(window).width() > 768) && $(".sidebar1").hasClass("toggled")) {
        $('.container1-fluid').removeClass( "sb1-notoggle" );
        $('.container1-fluid').addClass( "sb1-toggle" );
      }
      if (($(window).width() > 768) && !$(".sidebar1").hasClass("toggled")) {
        $('.container1-fluid').removeClass( "sb1-toggle" );
        $('.container1-fluid').addClass( "sb1-notoggle" );
      }
      if (($(window).width() < 768) && $(".sidebar1").hasClass("toggled")) {
        $('.container1-fluid').removeClass( "sb1-notoggle" );
        $('.container1-fluid').removeClass( "sb1-toggle" );
      }
      if (($(window).width() < 768) && !$(".sidebar1").hasClass("toggled")) {
        $('.container1-fluid').addClass( "sb1-toggle" );
        $('.container1-fluid').removeClass( "sb1-notoggle" );
      }

      // Toggle the side navigation when window is resized below 480px
      if ($(window).width() < 480 && !$(".sidebar").hasClass("toggled")) {
        $("body").addClass("sidebar-toggled");
        $(".sidebar").addClass("toggled");
        $('.sidebar .collapse').collapse('hide');
      }

      // Toggle the side navigation when window is resized below 480px
      if ($(window).width() < 480 && !$(".sidebar1").hasClass("toggled")) {
        $("body").addClass("sidebar1-toggled");
        $(".sidebar1").addClass("toggled");
        $('.sidebar1 .collapse').collapse('hide');
      }
  });

  // Prevent the content wrapper from scrolling when the fixed side navigation hovered over
  // $("body.fixed-nav .sidebar").on("mousewheel DOMMouseScroll wheel", function (
  //   e
  // ) {
  //   if ($(window).width() > 768) {
  //     var e0 = e.originalEvent,
  //       delta = e0.wheelDelta || -e0.detail;
  //     this.scrollTop += (delta < 0 ? 1 : -1) * 30;
  //     e.preventDefault();
  //   }
  // });

  // Scroll to top button appear
  // $(document).on("scroll", function () {
  //   var scrollDistance = $(this).scrollTop();
  //   if (scrollDistance > 100) {
  //   //if ($(this).scrollTop() > 100) {
  //     $(".scroll-to-top").fadeIn();
  //   } else {
  //     $(".scroll-to-top").fadeOut();
  //   }
  // });

  // Smooth scrolling using jQuery easing
  //$(document).on("click", "a.scroll-to-top", function (e) {
    //$('#table-top').animate({ scrollTop: "0px" });
    // var $anchor = $(this);
    // $("html, body")
    //   .stop()
    //   .animate(
    //     {
    //       scrollTop: $($anchor.attr("href")).offset().top,
    //     },
    //     1000,
    //     "easeInOutExpo"
    //   );
    // e.preventDefault();
  //});

  $('#table-top').on("scroll", function () {
    if ($(this).scrollTop() > 50) {
      $(".scroll-to-top").fadeIn();
    } else {
      $(".scroll-to-top").fadeOut();
    }
  });

  $('#table-top1').on("scroll", function () {
    if ($(this).scrollTop() > 50) {
      $(".scroll-to-top1").fadeIn();
    } else {
      $(".scroll-to-top1").fadeOut();
    }
  });


  // $(document).on("scroll", function () {
  //   var scrollDistance = table.scrollTop();
  //   if (scrollDistance > 50) {
  //     $(".scroll-to-top").fadeIn();
  //   } else {
  //     $(".scroll-to-top").fadeOut();
  //   }
  // });

  
})(jQuery); // End of use strict


