...
export class SlavesInfoComponent implements OnInit {
  ...
  getModelSlaveData() {
    this.modelSlaveData = [];
    const obj = {};
    this.subscriptions.push(this.deviceTypeService.getModelSlaveDetails(this.contextApp.app, this.device?.tags?.device_type, obj)
  ...
  getSlaveData() {
    ...
    const obj = {};
    this.subscriptions.push(this.deviceService.getAssetSlaveDetails(this.contextApp.app, this.device.device_id, obj)
    .subscribe((response: any) => {
      if (response.data) {
  ...
  setupFormData(obj = undefined) {
    console.log(obj);
    if (this.device.tags.protocol === 'ModbusTCPMaster') {
    ...
    } else if (this.device.tags.protocol === 'ModbusRTUMaster') {
      this.setupForm = new FormGroup({
    ...
    } else  if (this.device.tags.protocol === 'SiemensTCPIP') {
      this.setupForm = new FormGroup({
        host_address: new FormControl(
          (obj && obj.metadata && obj.metadata && obj.metadata.host_address !== undefined
  ...
  addSlaveObj() {
    ...
    this.slaveObj.asset_model = this.device?.tags?.device_type || this.device?.device_type;
    const macID = this.slaveObj.metadata.mac_id;
    this.slaveObj.metadata = this.setupForm?.value;
    this.slaveObj.metadata.mac_id = macID;
    this.subscriptions.push(this.deviceService.createAssetSlaveDetail(this.contextApp.app, this.device.device_id, this.slaveObj)
    .subscribe((response: any) => {
  ...
  updateSlaveObj(slave) {
    ...
    this.subscriptions.push(this.deviceService.updateAssetSlaveDetail(this.contextApp.app, this.device.device_id,
      slave.id, obj)
    .subscribe((response: any) => {
  ...
  deleteSlave() {
    ...
    this.subscriptions.push(this.deviceService.deleteAssetSlaveDetail(this.contextApp.app, this.device.device_id,
  ...
  syncSlavesWithGateway() {
    ...
    const c2dObj = {
      device_id: this.device.type !== CONSTANTS.NON_IP_DEVICE ? this.device.device_id : this.device.gateway_id,
      job_id: (this.device.type !== CONSTANTS.NON_IP_DEVICE ? this.device.device_id : this.device.gateway_id)
    ...
    this.subscriptions.push(
      this.deviceService.sendC2DMessage(c2dObj, this.contextApp.app,
        this.device.type !== CONSTANTS.NON_IP_DEVICE ? this.device.device_id : this.device.gateway_id).subscribe(
        (response: any) => {
