...
export class ChartWidgetComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.searchHistory(this.currentLayout).then((plottedChart: GoogleChartInterface) => {
      // const ccComponent = this.chartData.component;
      // ccComponent.draw();
      this.chartData = {...plottedChart}
      let showTable = this.currentLayout ? this.currentLayout.showDataTable : this.showDataTable
      if (showTable) {
  ...
  ngAfterViewInit() {
    if(this.isLayout){
      $("#" + this.chartId).draggable({ handle: ".card-header", containment: "#layoutWidgetContainer" })
      .resizable();
    }
    else{
  ...
  searchHistory(layoutJson) {
    ...
    return new Promise((resolve, reject) => {
      ...
      obj.message_props = '';
      let type = layoutJson ? layoutJson.chartType : this.type;
